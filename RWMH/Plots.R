...
################################################## Plots - CMH1
pdf(paste("As_RWMH.pdf"), height = 6, width = 18)
par(mfrow = c(1,3))
for(i in 1:length(h)){
  plot(l2, 1 - lb_list[[i]], type = "l", ylim = c(0, 1),
       ylab = "(1 - A*)", xlab = TeX(r'($X_2$)'), main = paste("h = ",h[i]), cex = 4)
}
dev.off()
...
ggsave(paste("A12_RWMH.pdf"), combined_plot, height = 8, width = 25, units = "in")
